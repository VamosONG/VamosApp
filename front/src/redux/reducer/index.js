...
const reducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case DELETE_DRIVER:
      return {
        ...state,
        conductores: state.conductores.filter(
    ...
    case PAGINATE:
      ...
      const firstIndex =
        action.payload === "next"
          ? nextPage * state.cantConductoresPorPag
          : prevPage * state.cantConductoresPorPag;
      ...
      return {
        ...state,
        pageConductores:
          /* [...state.pageConductores] */ state.conductores.splice(
            firstIndex,
            state.cantConductoresPorPag
          ),
    ...
    case ORDER_PASSENGER:
      ...
      if (action.payload === "A") {
        orderedListPassenger.sort((a, b) =>
          a.capacityPassengers < b.capacityPassengers ? 1 : -1
        );
      } else if (action.payload === "D") {
        orderedListPassenger.sort((a, b) =>
          a.capacityPassengers > b.capacityPassengers ? 1 : -1
    ...
    case FILTER_CAR:
      const filterCarList = state.allData.filter(
        (car) => car.carType === action.payload
      );
      return {
        ...state,
    ...
    case FILTER_AIRPORT:
      const filterAirportList = state.allData.filter(
        (zone) => zone.airports === action.payload
      );
